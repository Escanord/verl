...
Two additional components complete the algorithm. A per-rollout KL-in-reward penalty (Section 3.4) subtracts the mean per-token KL divergence from the base policy from the raw score prior to advantage normalization, suppressing a shortcut to which the Langevin perturbation is otherwise vulnerable: a perturbation can stochastically redirect the trajectory toward a brief correct answer, producing a high-advantage rollout whose policy gradient teaches the model to truncate reasoning. Short-collapsed rollouts drift far from the base distribution and therefore incur a large penalty, pushing them below longer correct rollouts in within-group ranking without imposing a hand-tuned target length. An importance-sampling correction (Section 3.5) integrates the off-policy trajectories produced under the perturbed rollout policy into the GRPO objective with bounded weight variance.
...
### 3.4 KL-in-Reward Length Stabilization
...
DRIFT addresses this by **subtracting a per-rollout mean-KL distributional-drift penalty from the raw score prior to advantage normalization** (InstructGPT-style reward shaping, Ouyang et al., 2022). The raw rollout score r^{i,j} is replaced by an adjusted score r̃^{i,j}:

    r̃^{i,j} = r^{i,j} − β_KL · (1 / L^{i,j}) · ∑_{t=1}^{L^{i,j}} KL̂( π_θ(· | x^i, ỹ^{i,j}_{<t}) ‖ π_ref(· | x^i, ỹ^{i,j}_{<t}) )

where L^{i,j} is the rollout length, π_ref is the pre-trained base policy, β_KL is the KL-in-reward coefficient, and KL̂ denotes the low-variance k=3 KL estimator (Schulman, 2020). The KL is aggregated as a **mean** over the rollout's tokens rather than a sum. The GRPO advantage is then computed from the adjusted scores:

    Â^{i,j} = (r̃^{i,j} − mean_j r̃^{i,j}) / std_j r̃^{i,j}.

**Why per-rollout mean-KL (not sum-KL).** A summed KL grows linearly with rollout length, penalizing long rollouts merely for length — the opposite of what a length-collapse defense should incentivize. Mean-KL is length-independent in aggregation: it responds to how far the policy has drifted *per token* from the base distribution, not to how many tokens the rollout contains. Empirically, short-collapsed rollouts (a handful of tokens emitting `\boxed{X}` directly) are highly unusual under the base policy — the base assigns negligible probability to such truncated completions — and therefore exhibit high per-token KL. Natural-length reasoning rollouts stay near the base distribution and incur negligible mean-KL. The adjustment thus targets the failure mode (short, off-distribution completions) while remaining agnostic to what "natural length" is for a given base model.

**Why in-reward rather than in-loss.** The standard GRPO/InstructGPT convention appends a KL term to the loss, L(θ) = L_PG + λ_KL · KL(π_θ ‖ π_ref), regularizing global distributional drift but leaving within-group advantage ranking untouched — mean-subtraction removes any additive shift common to a group. Only the in-reward form gives a **within-group** signal: because short-collapsed correct rollouts have distinctly higher per-token KL than the long-correct rollouts they compete against within a group, subtracting β_KL · mean-KL from the raw score pushes the short rollout below the long one in advantage rank, and the resulting policy gradient prefers longer correct reasoning. A separate in-loss KL regularizer with small coefficient λ_KL = 10⁻³ (Section 3.5) remains active to anchor global distributional drift, decoupled from the within-group reshaping performed here.

**Comparison to length-target penalties.** An earlier design of DRIFT used a hand-tuned length-target penalty r̃ = r + λ · (min(L / L_target, 1) − 1) · 𝟙[r > 0] with L_target = 1000 and λ = 2. That formulation required calibrating both an assumed target length and a coefficient that had to simultaneously suppress short collapses (L ≪ L_target) and mild length deficits (L ≈ L_target) — a tradeoff no single λ can satisfy uniformly; at fixed L_target = 1000, driving short-correct rollouts to negative within-group advantage requires λ ≥ 2.2 at L = 87 but λ ≥ 10 at L = 800. Replacing this length surrogate with per-rollout mean-KL eliminates both hyperparameters: distributional distance from the base policy is a scale-free, model-inferred proxy for "unusual" response profiles, and mean-KL matches short-collapsed rollouts (the intended target) without imposing a fixed L_target. This is particularly useful across model scales, where the base policy's characteristic reasoning length can differ substantially — the KL formulation adapts automatically while a fixed L_target would need model-specific retuning.
...
Input
  ...
  Hyperparameters
    β_KL                     KL-in-reward coefficient (per-rollout mean-KL)
    ...
    λ_KL                     in-loss KL regularization coefficient

# 1. KL-in-reward score adjustment (Section 3.4)
for each rollout (i, j):
    KL_mean^{i,j} ← (1 / L^{i,j}) · Σ_{t=1..L^{i,j}} KL̂( π_θ(· | x^i, ỹ^{i,j}_{<t}) ‖ π_ref(· | x^i, ỹ^{i,j}_{<t}) )
    r̃^{i,j}      ← r^{i,j} − β_KL · KL_mean^{i,j}
