#!/usr/bin/env bash
# Evaluate GRPO baseline on Qwen3-1.7B-Base.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

TAG_PREFIX="grpo_1p7b"
MODEL=/storage/workspace/server-1/duy/checkpoints/models/Qwen3-1.7B-Base
CKPT_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/grpo/qwen3_1p7b_base

EVAL_OUT_ROOT="${EVAL_OUT_ROOT:-/storage/workspace/server-1/duy/checkpoints/verl/eval_out}"
N_GPUS="${N_GPUS:-8}"
ROLLOUT_TP="${ROLLOUT_TP:-1}"
N_VAL="${N_VAL:-16}"
STEPS="${STEPS_1P7B:-20 40 60 80 100 120 140 160}"
