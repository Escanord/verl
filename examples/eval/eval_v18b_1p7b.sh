#!/usr/bin/env bash
# Evaluate DRIFT (v18b) on Qwen3-1.7B-Base.
#
# Sweeps STEPS_1P7B; each step runs verl val_only across all 4 benchmarks
# in one job (8 GPUs, vLLM + Ray).  Skips steps where the checkpoint or the
# JSONL output is already present.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

TAG_PREFIX="v18b_1p7b"
MODEL=/storage/workspace/server-1/duy/checkpoints/models/Qwen3-1.7B-Base
CKPT_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18b/qwen3_1p7b_base

EVAL_OUT_ROOT="${EVAL_OUT_ROOT:-/storage/workspace/server-1/duy/checkpoints/verl/eval_out}"
N_GPUS="${N_GPUS:-8}"
ROLLOUT_TP="${ROLLOUT_TP:-1}"
N_VAL="${N_VAL:-16}"
STEPS="${STEPS_1P7B:-20 40 60 80 100 120 140 160}"   # v18b 1.7B has 20-120; the rest are auto-skipped
...
# Aggregate per-(method, size) summary JSONL
...
python3 "${SCRIPT_DIR}/aggregate_eval.py" \
    ...
    --size "${SIZE}" \
    --format jsonl \
    --out "${SUMMARY_FILE}" || echo "[warn] aggregation skipped (no results yet?)"
