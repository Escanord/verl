#!/usr/bin/env bash
# Evaluate DRIFT (v18c) on Qwen3-4B-Base.
#
# v18c 4B has fewer saved checkpoints than the baselines because the run is
# still ongoing at the time of writing.  Default STEPS_4B=60 reflects the
# latest available save; rerun with STEPS_4B="60 80 100 ..." once those
# checkpoints exist.
set -euo pipefail
...
CKPT_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18c/qwen3_4b_base

EVAL_OUT_ROOT="${EVAL_OUT_ROOT:-/storage/workspace/server-1/duy/checkpoints/verl/eval_out}"
N_GPUS="${N_GPUS:-8}"
ROLLOUT_TP="${ROLLOUT_TP:-2}"   # 4B benefits from TP=2
N_VAL="${N_VAL:-16}"
STEPS="${STEPS_4B:-20 40 60 80 100 120 140 160}"   # v18c 4B currently has 40,60; rest auto-skipped
