#!/usr/bin/env bash
# Evaluate v15b step 180 with entropy-only trigger (no ΔVar).
# Langevin fires when H > entropy_threshold=8.0 on each rollout independently,
# matching real inference (n=1) behavior exactly.
#
# Contrast with:
#   eval_v15b_no_langevin.sh    — pure π sampling, no Langevin at all
#   training val metrics        — ΔVar trigger, Langevin-inflated
set -x
...
export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger

echo "==> Evaluating v15b checkpoint (entropy trigger): $RESUME_PATH"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_eval_v15b_entropy_trigger' \
    trainer.experiment_name="eval_v15b_entropy_trigger_${STEP}" \
    ...
    trainer.default_local_dir=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger \
    "$@"
