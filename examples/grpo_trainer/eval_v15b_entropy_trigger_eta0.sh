#!/usr/bin/env bash
# Evaluate v15b step 180: noise-only Langevin (η=0, σ=0.01).
# Zeroes out the entropy gradient push; logits_new = logits + ε only.
# Isolates whether the ∇H direction matters, or if it's just noise injection.
#
# Contrast with:
#   eval_v15b_entropy_trigger.sh          — η=0.1 σ=0.01 (baseline, 3.57%)
#   eval_v15b_entropy_trigger_sigma0.sh   — η=0.1 σ=0.0  (pure gradient, 2.81%)
#   eval_v15b_no_langevin.sh              — no Langevin at all (2.97%)
set -x
...
export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger-eta0

echo "==> Evaluating v15b checkpoint (noise-only, η=0 σ=0.01): $RESUME_PATH"
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.pivot.langevin_eta=0.0 \
    ...
    trainer.project_name='verl_eval_v15b_entropy_trigger_eta0' \
    trainer.experiment_name="eval_v15b_entropy_trigger_eta0_${STEP}" \
    ...
    trainer.default_local_dir=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger-eta0 \
    "$@"
