#!/usr/bin/env bash
# Evaluate v15b step 180 with entropy-only trigger (no ΔVar), K=2 Langevin steps.
# Two steps: each recomputes ∇H from the updated distribution, pushing further
# toward higher entropy than K=1.
#
# Contrast with:
#   eval_v15b_entropy_trigger.sh — same but K=1 (3.57% mean@16)
#   eval_v15b_no_langevin.sh     — pure π, no Langevin (2.97% mean@16)
set -x
...
export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger-k2

echo "==> Evaluating v15b checkpoint (entropy trigger K=2): $RESUME_PATH"
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.pivot.langevin_K=2 \
    ...
    trainer.project_name='verl_eval_v15b_entropy_trigger_k2' \
    trainer.experiment_name="eval_v15b_entropy_trigger_k2_${STEP}" \
    ...
    trainer.default_local_dir=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger-k2 \
    "$@"
