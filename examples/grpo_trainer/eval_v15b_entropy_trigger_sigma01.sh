#!/usr/bin/env bash
# Evaluate v15b step 180: high-noise Langevin (σ=0.1).
# 10× more stochastic noise than the baseline (σ=0.01).
# Tests whether extra randomness at fork points helps or hurts.
#
# Contrast with:
#   eval_v15b_entropy_trigger.sh        — σ=0.01 (baseline, 3.57% mean@16)
#   eval_v15b_entropy_trigger_sigma0.sh — σ=0.0 (pure gradient, no noise)
...
export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger-sigma01

echo "==> Evaluating v15b checkpoint (entropy trigger σ=0.1, high noise): $RESUME_PATH"
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.pivot.langevin_sigma=0.1 \
    ...
    trainer.project_name='verl_eval_v15b_entropy_trigger_sigma01' \
    trainer.experiment_name="eval_v15b_entropy_trigger_sigma01_${STEP}" \
    ...
    trainer.default_local_dir=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger-sigma01 \
    "$@"
