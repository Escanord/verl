#!/usr/bin/env bash
# Evaluate v15b step 180 with entropy-only trigger, threshold=9.5 (fires more selectively).
# Higher threshold → fewer positions trigger (~top-5% by entropy vs ~top-20% at 8.0).
set -x
...
export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger-thr95

echo "==> Evaluating v15b checkpoint (entropy trigger thr=9.5): $RESUME_PATH"
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.pivot.entropy_threshold=9.5 \
    ...
    trainer.project_name='verl_eval_v15b_entropy_trigger_thr95' \
    trainer.experiment_name="eval_v15b_entropy_trigger_thr95_${STEP}" \
    ...
    trainer.default_local_dir=/storage/workspace/server-1/duy/checkpoints/verl/eval-v15b-entropy-trigger-thr95 \
    "$@"
