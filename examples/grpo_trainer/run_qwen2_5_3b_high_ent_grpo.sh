#!/usr/bin/env bash
# GRPO + High-Entropy 80/20 on guru-RL-92k with Qwen2.5-3B-Instruct on 8x H200.
# Identical to run_qwen2_5_3b_guru_rl.sh except entropy_top_ratio=0.2:
# only the top 20% highest-entropy response tokens receive gradient updates.
#
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/high-ent-grpo/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/high-ent-grpo \
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_high_ent_grpo_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_high_ent_grpo' \
