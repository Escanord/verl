#!/usr/bin/env bash
# PIVOT-v10a seeded from GRPO step-200 checkpoint.
#
# Bipolar split_kl internalization.
#
...
#
# vs v8a (split_kl): same cm direction, but ALL rollouts enter the loss
# with sign from adv — winners attracted toward p_lan_b, losers repelled
# from p_lan_b.  v8a only attracted winners; losers only influenced cm.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v10a-from-grpo200/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v10a-from-grpo200
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.pivot.internalize_mode=split_kl_bipolar \
    ...
    trainer.project_name='verl_pivot_v10a_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v10a_from_grpo200' \
