#!/usr/bin/env bash
# PIVOT-v10b seeded from GRPO step-200 checkpoint.
#
# Bipolar contrastive_kl internalization.
#
#   cm(t) = (1/n) * sum_i adv_i * logits_i(t)   (GRPO-style soft weighting)
#   p_lan_b(t) = softmax((1-eta)*logits_b(t) + eta*cm(t))
#
#   L = sum_{b, t:trigger} adv(b) * KL(p_lan_b || pi_theta, top-20)
#
# vs v8b (contrastive_kl): same cm direction, but ALL rollouts enter the
# loss with sign from adv.  v8b only attracted winners; the cm already
# encoded loser repulsion but the loss gradient was sign-collapsed when
# Sigma(adv_i) ~ 0.  Bipolar gives losers an explicit repulsive gradient.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v10b-from-grpo200/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v10b-from-grpo200
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.pivot.internalize_mode=contrastive_kl_bipolar \
    ...
    trainer.project_name='verl_pivot_v10b_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v10b_from_grpo200' \
