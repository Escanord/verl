#!/usr/bin/env bash
# PIVOT-v13 seeded from GRPO step-200 checkpoint.
#
# Correct IS denominator — theoretically grounded Langevin-augmented GRPO.
#
# During rollout, trigger positions sample from p_lan_old, NOT π_old.
# The IS-correct policy gradient patches the denominator:
#
#   p_lan_approx(a_t) = (1-η)·π_current(a_t) + η·softmax(μ_group_current)(a_t)
#   (approximates p_lan_old; error is O(KL(π_old, π_current)), controlled by KL penalty)
#
#   L_trigger(t)     = adv * (log π_current(a_t) - log p_lan_approx(a_t).detach())
#   L_non-trigger(t) = adv * (log π_current(a_t) - log π_old(a_t))  [standard GRPO]
#
# vs v12 (lan_grpo numerator): patched log p_lan_current in numerator — SFT-like flavor.
# v13 keeps log π_current in numerator; only the denominator changes at trigger positions.
# Gradient flows purely through log π_current — no target imitation.
#
# Loop closure: at positions where Langevin boosted a_t (p_lan > π_old), the
# larger denominator shrinks the IS ratio → the policy must genuinely improve
# to get credit. Over time it learns those tokens without Langevin.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v13-from-grpo200/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v13-from-grpo200
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v13_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v13_from_grpo200' \
