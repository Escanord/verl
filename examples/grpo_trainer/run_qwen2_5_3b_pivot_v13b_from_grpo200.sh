#!/usr/bin/env bash
# PIVOT-v13b seeded from GRPO step-200 checkpoint.
#
# v13 (correct IS denominator) + v11b (codesign internalization).
#
#   L_trigger(t)     = adv * (log π_current(a_t) - log p_lan_approx(a_t).detach())
#                    + 0.01 * (1 - 2*sens(t)) * delta_var(t)
#   L_non-trigger(t) = adv * (log π_current(a_t) - log π_old(a_t))  [standard GRPO]
#
# The two trigger-position terms are orthogonal:
#   - IS correction operates on log π_θ(a_t) (sampled token log prob)
#   - Codesign operates on Var(logits across rollouts) at the trigger position
#
# Codesign calibrates which positions are real forks (high sensitivity →
# high delta_var → more likely to trigger). IS correction ensures the RL
# signal is theoretically grounded at those positions.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v13b-from-grpo200/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v13b-from-grpo200
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v13b_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v13b_from_grpo200' \
