#!/usr/bin/env bash
# PIVOT-v13 + adaptive threshold, seeded from GRPO step-200 checkpoint.
#
# v13 (IS-correct denominator) + trig_percentile=90: always trigger the top
# 10% of positions by delta_var in the running buffer, regardless of how the
# distribution shifts as the policy improves.
#
# Motivation: with a fixed delta_var_threshold=9.0 the trigger rate drifts
# from ~10% at step 1 to ~40% by step 120 as delta_var shrinks. This inflates
# the IS correction at positions that are no longer genuinely uncertain, adding
# noise rather than signal.
#
# trig_percentile=90 locks trig_frac ≈ 0.10 throughout training.
# Rollout and actor both use _agg_delta_vars[-2000:] / _actor_pivot_dv_buf[-2000:]
# to compute the threshold; fallback to delta_var_threshold=9.0 for the first
# 200 samples.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v13c-from-grpo200/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v13c-from-grpo200
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v13c_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v13c_from_grpo200' \
