#!/usr/bin/env bash
# PIVOT-v14 seeded from GRPO step-200 checkpoint.
#
# v13 (correct IS denominator) + fork-weighted advantage.
#
# At trigger positions (delta_var > threshold), the token choice causally
# determines whether the trajectory succeeds or fails — this is the fork.
# Standard GRPO treats all positions equally, diluting the gradient signal
# at the positions that matter most.
#
# Fork-weighted advantage amplifies the GRPO advantage at trigger positions:
#
#   A_t_fork = A_i * (1 + fork_advantage_alpha)  at trigger positions
#   A_t      = A_i                                elsewhere
#
# With alpha=1.0: trigger positions get 2x gradient. The policy learns
# to be more decisive at genuine forks → higher mean@k consistency.
#
# IS correction (v13) remains: correct denominator at trigger positions
# ensures the amplified gradient is theoretically grounded.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v14-from-grpo200/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v14-from-grpo200
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.pivot.fork_advantage_alpha=1.0 \
    ...
    trainer.experiment_name='qwen2_5_3b_pivot_v14_from_grpo200' \
