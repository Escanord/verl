#!/usr/bin/env bash
# PIVOT-v15 seeded from GRPO step-200 checkpoint.
#
# v13d + direct GRPO term at trigger positions.
#
# Problem in v13/v13d: the IS-corrected denominator is log p_lan_old, where
# p_lan_old = (1-η)·π_old + η·softmax(μ_correct).  At fork positions where
# Langevin pushed strongly, p_lan_old(correct) >> π_old(correct), so the PPO
# clipping ratio r = π_current / p_lan_old << 1.  When r < 1-ε, the PPO clip
# zeroes the gradient — the IS correction is silencing the signal at exactly
# the positions that matter most for mean@k.
#
# v15 adds a direct GRPO term alongside the IS-corrected term:
#
#   loss_trig = adv·(log π_current - log p_lan_old)   [IS term, v13d]
#             + λ·adv·(log π_current - log π_old)      [direct term, ratio ≈ 1]
#
# The direct term uses the original π_old as denominator → ratio ≈ 1 throughout
# → PPO clip never engages → π_current always receives a strong gradient at
# fork positions regardless of how much Langevin perturbed the denominator.
#
# lan_grpo_direct_coeff=1.0 means equal weight to IS and direct terms.
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15-from-grpo200/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15-from-grpo200
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.pivot.lan_grpo_direct_coeff=1.0 \
    ...
    trainer.project_name='verl_pivot_v15_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v15_from_grpo200' \
