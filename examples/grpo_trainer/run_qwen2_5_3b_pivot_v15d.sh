#!/usr/bin/env bash
# PIVOT-v15d trained from scratch (Qwen2.5-3B-Instruct base).
#
# v15c but replacing the ΔVar group trigger with per-rollout entropy trigger.
#
# v15c fires Langevin when delta_var > adaptive(trig_percentile=80) threshold,
# requiring group-level ΔVar tracking across concurrent rollouts.
# v15d fires when H[P(·|context_t)] > entropy_threshold=1.0, using only the
# current rollout's own logit distribution — no inter-rollout communication.
#
# The actor-side IS loss uses entropy_trigger_only=True + entropy_threshold=1.0
# to match the rollout trigger exactly (mean_ent per position ≈ per-rollout H).
#
# All other hyper-parameters identical to v15c.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15d/qwen2_5_3b
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v15d_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v15d' \
