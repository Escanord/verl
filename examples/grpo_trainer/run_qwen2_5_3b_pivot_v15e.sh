#!/usr/bin/env bash
# PIVOT-v15e trained from scratch (Qwen2.5-3B-Instruct base).
#
# v15d + lan_grpo_restrict_to_trigger=True.
#
# v15d fires Langevin at top-20% entropy positions (per-rollout H > adaptive
# threshold) and applies IS-corrected GRPO at those same positions, but the
# base pg_loss still runs over all 100% of response tokens.
#
# v15e restricts pg_loss to exactly the Langevin-triggered positions:
#   response_mask &= lan_grpo_mask
# So gradient flows only where Langevin fired — Langevin, IS correction, and
# policy gradient are all perfectly aligned on the same ~20% of tokens.
# The bottom 80% (low-entropy, already-confident positions) get zero gradient,
# preventing entropy collapse without a separate entropy_top_ratio mask.
#
# All other hyper-parameters identical to v15d.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15e/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15e
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v15e_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v15e' \
