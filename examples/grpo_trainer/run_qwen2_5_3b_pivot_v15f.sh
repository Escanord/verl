#!/usr/bin/env bash
# PIVOT-v15f: v15e + momentum Langevin (underdamped / kinetic Langevin).
#
...
#
# v15f adds sequential coherence: a velocity vector v ∈ R^|vocab| persists
# across trigger positions within each generated sequence.
#
#   v_t = γ·v_{t-1} + (η·∇H_t + σ·noise)
#   logits_t += v_t
#
# γ=0.9 means a push at position t still has ~35% influence six triggers later,
# giving correlated directional exploration along a coherent reasoning branch
# rather than independent random jitter at each forking point.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15f/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15f
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.pivot.langevin_momentum=0.9 \
    ...
    trainer.project_name='verl_pivot_v15f_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v15f' \
