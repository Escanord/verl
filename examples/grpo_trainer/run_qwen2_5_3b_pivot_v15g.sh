#!/usr/bin/env bash
# PIVOT-v15g: v15e + MALA correction on the Langevin step.
#
# v15e fires Langevin at top-20% entropy positions and restricts pg_loss to
# exactly those positions (IS correction, Langevin, and gradient all aligned).
#
# v15g adds a Metropolis-Hastings accept/reject to each Langevin step:
#
#   propose: logits' = logits + η·∇H + σ·noise
#   log α = [H(logits') - H(logits)] + [||logits - logits' - η·∇H(logits')||²
#                                        - ||noise||²] / (2σ²)
#   accept logits' with probability min(1, exp(α)), else keep logits
#
# MALA guarantees exact samples from p_target ∝ exp(H(softmax(·))) and allows
# larger step sizes (langevin_eta=0.3 vs 0.1 in v15e) because bad steps are
# rejected rather than accumulated.  Acceptance rate ~70-85% in the top-k=20
# restricted logit space.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15g/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15g
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.pivot.entropy_threshold=1.0 \
    ...
    +actor_rollout_ref.rollout.pivot.langevin_mala=True \
    +actor_rollout_ref.rollout.pivot.langevin_min_trigger_position=300 \
    ...
    +actor_rollout_ref.actor.pivot.lan_grpo_restrict_to_trigger=True \
    +actor_rollout_ref.actor.pivot.langevin_eta=0.3 \
    ...
    trainer.project_name='verl_pivot_v15g_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v15g' \
