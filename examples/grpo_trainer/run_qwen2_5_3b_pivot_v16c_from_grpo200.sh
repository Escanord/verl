#!/usr/bin/env bash
# PIVOT-v16c seeded from GRPO step-200 checkpoint.
#
# v16 + trig_percentile=80 so IS addon fires on ~20% of response positions.
# No entropy masking (no entropy_top_ratio).
#
# v16 uses trig_percentile=90 → top-10% delta_var positions trigger (~10%).
# v16c uses trig_percentile=80 → top-20% delta_var positions trigger (~20%).
#
# Motivation: the 80/20 rule (high-ent) updates 20% of positions by entropy.
# v16c tests the same 20% budget but selected by Langevin delta_var instead of
# entropy — fork positions where Langevin caused the largest distribution shift.
# IS addon fires on all 20% triggered positions with full gradient.
#
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v16c-from-grpo200/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v16c-from-grpo200
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v16c_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v16c_from_grpo200' \
