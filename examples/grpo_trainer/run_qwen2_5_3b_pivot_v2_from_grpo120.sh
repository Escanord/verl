#!/usr/bin/env bash
# PIVOT-v2 seeded from GRPO step-120 checkpoint.
#
# Motivation: PIVOT-v2's co-design loop requires correct rollouts to bootstrap
# the v[t] = mean(h_t|correct) - mean(h_t|wrong) signal (Phase 1) and a
# post-breakthrough model whose generation entropy at routine positions is low
# enough (~0.5-1.5 nats) that entropy_threshold=2.0 correctly identifies
# genuine branch points rather than firing on everything.
#
# Starting from GRPO-320 (first breakthrough (~35% MATH500, 0.8% AIME) — MATH500 / 1.8% AIME) satisfies
# both conditions from step 0, making this a clean test of whether PIVOT-v2
# can break the GRPO plateau.
#
# See docs/algo/pivot.md for full design rationale.
set -x

DATA_DIR="${DATA_DIR:-/storage/workspace/server-1/duy/data/guru_rl}"
# GRPO step-320 weights merged to HF format
MODEL="${MODEL:-/storage/workspace/server-1/duy/checkpoints/verl/grpo/qwen2_5_3b/global_step_120/actor/huggingface_merged}"
REWARD_FN="$(dirname "$0")/guru_rl_reward.py"

train_files="$DATA_DIR/train.parquet"
val_files="['$DATA_DIR/test_aime.parquet','$DATA_DIR/test_math500.parquet']"

CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v2-from-grpo120/qwen2_5_3b
...
VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v2-from-grpo120 \
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen2_5_3b_pivot_v2_from_grpo120' \
