#!/usr/bin/env bash
# PIVOT-v4b seeded from GRPO step-200 checkpoint.
#
# True v4: group-mean Langevin rollout only, no walk advantage weighting,
# no entropy loss. Clean ablation of the Langevin rollout in isolation.
#
# Rollout:
#   logits_new = (1 - eta) * logits + eta * group_mean + sigma * noise
#   triggered when delta_var > delta_var_threshold
#
# Loss: standard GRPO + KL (no entropy bonus, no walk reweighting)
#
# Compare against:
#   - v4 (mislabeled): walk advantage weighting only, no real Langevin
#   - v5a: v4b + trigger-gated entropy loss
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v4b-from-grpo200/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v4b-from-grpo200
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v4b_guru_rl' \
    trainer.experiment_name='qwen2_5_3b_pivot_v4b_from_grpo200' \
