#!/usr/bin/env bash
# WGRPO-v2: Walk-Weighted GRPO with ReLU(z-score) normalization.
#
# Change vs WGRPO-v1: walk importance uses ReLU(z-score) instead of raw z-score.
#   - Scores in [0, inf): below-average tokens unaffected (weight=1), only hubs amplified.
#   - Eliminates the sign-flip issue and below-average token dampening of v1.
#
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/wgrpo-v2/qwen2_5_3b

source ~/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/wgrpo-v2 \
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen2_5_3b_wgrpo_v2' \
