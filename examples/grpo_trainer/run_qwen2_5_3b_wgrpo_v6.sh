#!/usr/bin/env bash
# WGRPO-v6: Differential Walk-Weighted GRPO (Option A).
#
# Builds on v5's walk computation (direct QK dot product, causal aggregation)
# but replaces per-rollout absolute hub scores with reward-contrastive scores:
#
#   delta_wi[t] = ReLU(mean_{above-avg-reward} wi[t] - mean_{below-avg-reward} wi[t])
#
# For each prompt group of n=8 rollouts, structural tokens that are equally
# attended to in both correct and wrong rollouts cancel out in the difference.
# Only tokens whose attention structure specifically differs between correct and
# wrong solutions get nonzero scores — filtering out trivial prompt anchors and
# formatting tokens that score high in absolute walk regardless of reasoning.
#
# Falls back to absolute walk scores for groups with uniform reward (no contrast),
# matching GRPO's zero-advantage case.
#
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/wgrpo-v6/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/wgrpo-v6 \
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen2_5_3b_wgrpo_v6' \
