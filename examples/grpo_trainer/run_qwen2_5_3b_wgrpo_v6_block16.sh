#!/usr/bin/env bash
# WGRPO-v6-block16: v6 differential walk with block_size=16 (down from 32).
#
# Identical to v6 in all respects except block_size=16.
# Two hypotheses being tested:
#   1. Stability: is v6's performance reproducible from scratch?
#   2. Resolution: does finer block granularity (16 vs 32 tokens/block) improve
#      fork-position precision and downstream AIME/MATH performance?
#
# See docs/algo/wgrpo.md for v6 design and results baseline.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/wgrpo-v6-block16/qwen2_5_3b

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/wgrpo-v6-block16 \
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.walk_importance.block_size=16 \
    ...
    trainer.experiment_name='qwen2_5_3b_wgrpo_v6_block16' \
