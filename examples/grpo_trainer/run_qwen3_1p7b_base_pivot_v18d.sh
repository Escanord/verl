#!/usr/bin/env bash
# PIVOT-v18d on Qwen3-1.7B-Base.
#
# v18d increases langevin_top_k from 20 → 150 to allow Langevin to reach
# reasoning pivot tokens ("revisit", "wait", "actually") that are ranked
# outside the top-20 but within the plausible vocabulary at trigger positions.
# All other hyperparameters identical to v18c.
set -x

DATA_DIR="${DATA_DIR:-/storage/workspace/server-1/duy/data/guru_rl}"
MODEL="${MODEL:-/storage/workspace/server-1/duy/checkpoints/models/Qwen3-1.7B-Base}"
REWARD_FN="$(dirname "$0")/guru_rl_reward.py"

train_files="$DATA_DIR/train.parquet"
val_files="['$DATA_DIR/test_aime.parquet','$DATA_DIR/test_math500.parquet']"

V18B_CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18b/qwen3_1p7b_base
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18d/qwen3_1p7b_base

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18d
export VLLM_FLASH_ATTN_VERSION=2
export VLLM_ATTENTION_BACKEND=FLASH_ATTN
python3 -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    algorithm.use_kl_in_reward=False \
    +algorithm.length_penalty_coef=2.0 \
    +algorithm.length_penalty_min_len=1000 \
    ...
    +actor_rollout_ref.rollout.pivot.langevin_top_k=150 \
    ...
    +actor_rollout_ref.actor.pivot.langevin_top_k=150 \
    ...
    trainer.project_name='verl_pivot_v18d_qwen3_guru_rl' \
    trainer.experiment_name='qwen3_1p7b_base_pivot_v18d' \
    ...
    trainer.resume_from_path="$V18B_CKPT_DIR/global_step_20" \
    "$@"
