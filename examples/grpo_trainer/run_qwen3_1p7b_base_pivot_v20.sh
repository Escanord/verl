#!/usr/bin/env bash
# PIVOT-v20 (DRIFT-v20) on Qwen3-1.7B-Base.
#
# v20 keeps v19's EOS mask and replaces v19's window-based adaptive t_min with
# a *monotonic-peak* t_min that fixes v19's late-training collapse pathology:
#
#   t_min(step) = clamp(alpha * peak_length, floor, cap)
#   peak_length = max(peak_length, current_median_length)   # monotonic, never decreases
#
# Rationale (from v19 empirical analysis):
#   - Langevin is helpful during bootstrap + healthy post-breakthrough phase
#     (faster bootstrap, higher AIME best@16 ceiling vs plain GRPO).
#   - Langevin is harmful once the policy enters a short-answer basin
#     (extends sampling distribution to include short-correct mode, which GRPO
#     then amplifies).
#   - v19's "median - W_min" adaptive t_min DROPS with length, so once collapse
#     begins Langevin keeps firing in the collapsed regime and reinforces it.
#   - v20's monotonic peak t_min RATCHETS UP with length, so collapse onset
#     drives the rollout length BELOW t_min -> Langevin auto-disables -> GRPO
#     can recover.  Automates the manual "pump t_min up at collapse onset"
#     intervention that rescued v18b 1.7B.
#
# CUDA graph (lan_use_cuda_graph) stays off because EOS mask and peak t_min
# logic are not in the captured kernel; falls back to non-graph adapter.
#
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v20/qwen3_1p7b_base

source /home/escanord/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/home/escanord/duy/checkpoints/verl/pivot-v20
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v20_qwen3_guru_rl' \
    trainer.experiment_name='qwen3_1p7b_base_pivot_v20' \
