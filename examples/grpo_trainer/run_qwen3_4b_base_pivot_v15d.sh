#!/usr/bin/env bash
# PIVOT-v15d on Qwen3-4B-Base (base model, no instruction tuning).
#
# Identical algorithm to run_qwen2_5_3b_pivot_v15d.sh:
#   - Entropy trigger (per-rollout H > adaptive 80th-pct threshold)
#   - IS-corrected GRPO at triggered positions
#   - langevin_min_trigger_position=250: skip first 250 response tokens so
#     Langevin does not perturb structural decision tokens (response format),
#     preventing catastrophic collapse to shortcut answers.
#
# After the _grp_trig memory fix (pre-computed IS denominators from rollout),
# PIVOT memory usage drops to GRPO-level, so we can use the same settings
# as the 3B baseline but with slightly reduced micro_batch for the larger model.
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15d/qwen3_4b_base

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15d
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.pivot.entropy_threshold=1.5 \
    ...
    +actor_rollout_ref.rollout.pivot.langevin_min_trigger_position=250 \
    ...
    trainer.project_name='verl_pivot_v15d_qwen3_guru_rl' \
    trainer.experiment_name='qwen3_4b_base_pivot_v15d' \
