#!/usr/bin/env bash
# PIVOT-v15h on Qwen3-4B-Base.
#
# v15f + Adam-style RMS normalization on the momentum Langevin step,
# seeded from PIVOT-v15f global_step_40 checkpoint, gate=250 tokens,
# trig_percentile=65 (targeting ~20% trig_frac).
#
#   step_t  = η·∇H_t + σ·noise
#   v_t     = γ·v_{t-1} + step_t
#   s_t     = β₂·s_{t-1} + (1-β₂)·step_t²
#   logits_t += v_t / (√s_t + ε)
#
# Without normalization (v15f), v_t can grow unboundedly if ∇H is consistently
# large in one direction. The RMS denominator bounds the effective step size
# regardless of momentum accumulation — same mechanism as Adam vs SGD+momentum.
#
# Seeded from v15f step-40 (mid dead-zone, stable entropy ~0.46) to skip the
# dead zone. trig_percentile lowered from 80→65 to target ~20% trig_frac
# (v15f observed ~74% calibration efficiency: 35% eligible × 73% eligible_frac
# × 74% efficiency ≈ 20%).
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15h/qwen3_4b_base

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15h
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v15h_qwen3_guru_rl' \
    trainer.experiment_name='qwen3_4b_base_pivot_v15h' \
    ...
    trainer.resume_from_path=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v15f/qwen3_4b_base/global_step_40 \
    "$@"
