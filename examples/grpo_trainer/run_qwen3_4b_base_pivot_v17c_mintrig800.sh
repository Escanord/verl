#!/usr/bin/env bash
# PIVOT-v17c resumed from step 80 with langevin_min_trigger_position=800.
#
# Hypothesis: raising the min trigger position from 200 to 800 breaks the
# Langevin shortcut-answer feedback loop (which fires at positions 200-600
# on format/structure tokens) while preserving Langevin exploration on the
# deep problem-solving phase of longer responses (position 800+). Expected
# effect: resp_len ceiling rises toward GRPO's 2000+ range while trig_frac
# stays meaningful at the positions that actually matter for hard problems.
#
# Base: pivot-v17c config (pivot_version=2, langevin_momentum=0.7,
#       langevin_exploit_ratio=0.6, no Adam-RMS).
# Change vs v17c: langevin_min_trigger_position 200 → 800.
# Resumed from: pivot-v17c/qwen3_4b_base/global_step_80
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v17c-mintrig800/qwen3_4b_base
RESUME_FROM=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v17c/qwen3_4b_base/global_step_80

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v17c-mintrig800
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v17c_mintrig800_qwen3_guru_rl' \
    trainer.experiment_name='qwen3_4b_base_pivot_v17c_mintrig800' \
