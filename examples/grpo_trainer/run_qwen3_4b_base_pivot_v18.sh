#!/usr/bin/env bash
# PIVOT-v18 on Qwen3-4B-Base.
#
# Two fixes over v17c:
#
# 1. Asymmetric IS correction (loss side):
#    Negative-advantage trigger positions use min(log_p_lan, log_π_old) as
#    denominator → ratio recovers to standard GRPO strength for wrong committed
#    paths. Positive-advantage positions keep the v17c blend unchanged.
#    Enabled via: lan_grpo_asym_is=True
#
# 2. Target-band G signal (rollout side):
#    Replaces median(H_past) - H_current with H_current - H_first*alpha.
#    G commits when H > H_target, stops near H_target, reverses if overshot.
#    H_first is the entropy at the first trigger this trajectory (model-agnostic).
#    alpha=0.7 allows 30% entropy decay from trajectory start before reversing.
#    Enabled via: langevin_alpha_target=0.7
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18/qwen3_4b_base

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.pivot.langevin_min_trigger_position=1000 \
    ...
    trainer.project_name='verl_pivot_v18_qwen3_guru_rl' \
    trainer.experiment_name='qwen3_4b_base_pivot_v18' \
