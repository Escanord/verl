#!/usr/bin/env bash
# PIVOT-v18b on Qwen3-4B-Base.
#
# Same algorithm as v18 with one implementation change:
#   lan_use_cuda_graph=True  →  PIVOTv18bLangevinAdapter (CUDA graph path).
#
# v18b changes vs v18:
...
# - G is updated BEFORE graph replay so Langevin uses the latest G momentum.
# - entropy_threshold is fixed from config (trig_percentile adaptive path not
#   wired into graph; each proc still uses its own proc.entropy_threshold for
#   the trigger decision but the graph uses the config fixed threshold).
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18b/qwen3_4b_base

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18b
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v18b_qwen3_guru_rl' \
    trainer.experiment_name='qwen3_4b_base_pivot_v18b' \
