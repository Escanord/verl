#!/usr/bin/env bash
# PIVOT-v18c on Qwen3-4B-Base.
#
# v18c adds length penalization in the advantage to combat length collapse:
#
#   penalty = lp_coef * (min(resp_len / lp_min_len, 1.0) - 1.0)
#
# Added to group scores BEFORE GRPO normalization, so short responses score
# worse relative to long ones within the same prompt group.
# lp_coef=0.5, lp_min_len=800 → penalty range [-0.5, 0].
#
# Also reverts langevin_min_trigger_position to 200 (baseline) since pumping it
# to 800/1000 had zero effect on collapse timing (always step 29-31).
set -x
...
V18_CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18/qwen3_4b_base
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18c/qwen3_4b_base

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v18c
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v18c_qwen3_guru_rl' \
    trainer.experiment_name='qwen3_4b_base_pivot_v18c' \
    ...
    trainer.resume_from_path="$V18_CKPT_DIR/global_step_20" \
    "$@"
