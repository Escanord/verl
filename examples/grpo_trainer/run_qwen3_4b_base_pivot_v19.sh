#!/usr/bin/env bash
# PIVOT-v19 (DRIFT-v19) on Qwen3-4B-Base.
#
# v19 = v18c structure with two surgical changes targeting length collapse:
...
#      regime (median ~1000-1500), this lands t_min in [600, 1100], replicating
#      the empirical 1.7B fix where pumping t_min ~800 stopped collapse.
#
# v18c's length penalty (length_penalty_coef=2.0, length_penalty_min_len=1000)
# is REMOVED — the goal is to verify that (1) + (2) prevent the collapse on
# their own.  The "hard floor on short correct" advantage adjustment is
# deferred to a future variant (see docs/drift_method.md §3.4 future work).
#
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v19/qwen3_4b_base
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen3_4b_base_pivot_v19' \
