#!/usr/bin/env bash
# PIVOT-v20 (DRIFT-v20) on Qwen3-4B-Base.
#
# v20 keeps v19's EOS mask and replaces v19's window-based adaptive t_min with
# a *monotonic-peak* t_min that fixes v19's collapse pathology on 4B:
#
...
#   - On 4B, v19 collapsed catastrophically at step 35-50 (length 1478 -> 151),
#     plateaued at length 30-100 with MATH ~30% AIME ~0% for ~200 steps, then
#     spontaneously recovered (length back to 465 by step 315, MATH 62%).
#   - v19's window-based t_min DROPS with length, so Langevin keeps firing in
#     the collapsed regime and reinforces the short-answer mode.
#   - v20's monotonic peak t_min RATCHETS UP with length, so when length drops
#     below alpha*peak, Langevin auto-disables -> the policy can recover via
#     plain GRPO without further Langevin-driven exploration of the shortcut.
#   - Goal: avoid the 200-step plateau in the collapsed basin entirely.
#
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v20/qwen3_4b_base
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen3_4b_base_pivot_v20' \
