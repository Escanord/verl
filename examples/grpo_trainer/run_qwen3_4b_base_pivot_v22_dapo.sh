#!/usr/bin/env bash
# PIVOT-v22 + DAPO on Qwen3-4B-Base.
#
...
#   - InstructGPT-style per-rollout MEAN-KL reward shaping (β=0.2, kl_ctrl.kl_coef)
#   - actor kl_loss_coef=0.001, use_kl_loss=True (light KL pull)
#   - clip_ratio_low=0.2, clip_ratio_high=0.28 (DAPO-style asymmetric clip)
#
# From DAPO (ADDED):
...
#   - loss_agg_mode=token-mean (DAPO loss aggregation)
#   - reward_manager=dapo (overlong soft-length reward shaping)
#
# Rationale: v22's Langevin can break the dead zone (find correct rollouts even
# at 0% base accuracy) where pure DAPO's filter_groups would terminate from
# max_num_gen_batches exhaustion. DAPO's filter_groups then guarantees reward
# variance once Langevin has seeded enough correct rollouts.
set -x
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v22-dapo/qwen3_4b_base
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen3_4b_base_pivot_v22_dapo' \
