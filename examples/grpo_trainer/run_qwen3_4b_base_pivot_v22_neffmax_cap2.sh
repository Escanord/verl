#!/usr/bin/env bash
# DRIFT-v22 neffmax on Qwen3-4B-Base — soft-IS with a raised ceiling (cap=2).
# Identical to run_qwen3_4b_base_pivot_v22_neffmax.sh EXCEPT the soft-IS
# weight ceiling is raised from 1 to 2:
#   - lan_grpo_softis_wmax=2.0  =>  w = min(2, π_old/π_lan_old). Allows the
#     off-policy correction to UP-weight (w>1) the tokens the policy favours
#     more than the perturbed rollout did, instead of capping them at 1, while
#     still bounding the tail variance (the fully-uncapped w hit ~1e7 and
#     diverged). Less conservative than the default min(1,·).
#   - N_eff BAND trigger: fire iff 2 <= 1/Σp² <= 8. langevin_exploit_ratio=1.0.
set -x
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v22/qwen3_4b_base_neffmax_cap2
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="qwen3_4b_base_pivot_v22_neffmax_cap2_${SLURM_JOB_ID:-manual}" \
