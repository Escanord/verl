#!/usr/bin/env bash
# DRIFT-v22 neffmax_cap2 + High-Entropy GRPO (entropy_top_ratio=0.2) on Qwen3-4B-Base.
# 4B analogue of run_qwen3_8b_base_pivot_v22_neffmax_cap2_heg.sh: stacks DRIFT's
# soft-IS advantage reweight (ceiling=2) on top of heg's top-20%-entropy token
# masking. Fresh run (no resume). Matched to grpo/cap2 4B (resp_len 16384, batch
# 1024, KL-in-reward OFF, clip 0.2-0.2, entropy_coeff=0).
#   - N_eff BAND trigger: fire iff 2 <= 1/Σp² <= 8. langevin_exploit_ratio=1.0.
#   - soft-IS: w = min(2, π_old/π_lan_old)  (lan_grpo_softis_wmax=2.0).
#   - heg: entropy_top_ratio=0.2 masks policy-gradient to top-20%-entropy tokens.
set -x
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v22/qwen3_4b_base_neffmax_cap2_heg
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="qwen3_4b_base_pivot_v22_neffmax_cap2_heg_${SLURM_JOB_ID:-manual}" \
