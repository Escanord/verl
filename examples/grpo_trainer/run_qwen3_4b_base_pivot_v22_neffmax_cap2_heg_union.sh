#!/usr/bin/env bash
# GRPO + heg + DRIFT-v22 neffmax_cap2 on Qwen3-4B-Base — UNION integration.
# Clone of run_qwen3_4b_base_pivot_v22_neffmax_cap2_heg.sh (intersection) with the
# new union flag: gradient acts on (top-20%-entropy tokens  UNION  DRIFT soft-IS
# trigger tokens) instead of their intersection, via
#   +actor_rollout_ref.actor.entropy_top_union_trigger=True
# so DRIFT-drifted trigger tokens still receive gradient even when they are not
# in heg's top-entropy set (and vice-versa). Fresh run. Everything else matched to
# the intersection run (resp_len 16384, batch 1024, KL-in-reward OFF, clip 0.2-0.2).
set -x
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v22/qwen3_4b_base_neffmax_cap2_heg_union
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="qwen3_4b_base_pivot_v22_neffmax_cap2_heg_union_${SLURM_JOB_ID:-manual}" \
