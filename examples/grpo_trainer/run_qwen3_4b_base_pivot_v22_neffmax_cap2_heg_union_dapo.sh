#!/usr/bin/env bash
# DAPO + heg + DRIFT-v22 neffmax_cap2 on Qwen3-4B-Base — UNION integration.
# Clone of run_qwen3_4b_base_pivot_v22_neffmax_cap2_dapo.sh with heg added on top:
#   +actor_rollout_ref.actor.entropy_top_ratio=0.2         (top-20%-entropy masking)
#   +actor_rollout_ref.actor.entropy_top_union_trigger=True (gradient on top-ent UNION trigger)
# Base = paper-scale DAPO (20480 resp, batch 512, filter_groups gen 3x, clip
# 0.2/0.28/c=10, token-mean, dapo reward_manager w/ overlong 4096, n=16, NO KL)
# + neffmax_cap2 DRIFT (N_eff band 2..8, exploit_ratio=1.0, top_k=128, t_min=400,
# soft-IS w=min(2, pi_old/pi_lan_old)). train data = guru_rl. Fresh run.
set -x
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v22-dapo/qwen3_4b_base_neffmax_cap2_heg_union
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name="qwen3_4b_base_pivot_v22_neffmax_cap2_heg_union_dapo_${SLURM_JOB_ID:-manual}" \
