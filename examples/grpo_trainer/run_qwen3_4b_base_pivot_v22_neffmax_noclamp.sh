#!/usr/bin/env bash
# DRIFT-v22 neffmax on Qwen3-4B-Base — UNCLAMPED soft-IS variant.
# Identical to run_qwen3_4b_base_pivot_v22_neffmax.sh EXCEPT the soft-IS
# weight is no longer truncated to <=1:
#   - lan_grpo_softis_clamp=False  =>  w = π_old/π_lan_old  (exact, unbiased;
#     can exceed 1). Removes the one-sided down-weight / conservative bias of
#     the min(1,·) clamp. Higher variance; may be less stable.
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v22/qwen3_4b_base_neffmax_noclamp
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.pivot.lan_grpo_softis_clamp=False \
    ...
    trainer.experiment_name="qwen3_4b_base_pivot_v22_neffmax_noclamp_${SLURM_JOB_ID:-manual}" \
