#!/usr/bin/env bash
# DRIFT-v22 on Qwen3-4B-Base, guru_rl math — REBUTTAL run, matched 1:1 to
# run_qwen3_4b_base_grpo_rebuttal.sh so the only difference is DRIFT on/off.
#   - identical data / reward (fixed strict-box \boxed{}) / batch 1024 / KL / clip
#   - symmetric clip 0.2/0.2 (NOT DAPO clip-higher) — matches the GRPO baseline
#   - KL matches GRPO (use_kl_in_reward=False) — no extra in-reward KL
#   - peak-mode trigger gate using the RENAMED unified params:
#       langevin_tmin (=floor in peak mode), langevin_tmin_cap,
#       langevin_tmin_peak_alpha, langevin_tmin_mode
#   - langevin_top_k=128 (experimental; watch resp_len / mean@16 for collapse)
#   - langevin_exploit_ratio=0.9
set -x
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v22/qwen3_4b_base_rebuttal
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='qwen3_4b_base_pivot_v22_rebuttal' \
