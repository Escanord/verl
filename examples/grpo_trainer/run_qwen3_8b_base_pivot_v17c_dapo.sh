#!/usr/bin/env bash
# PIVOT-v17c-dapo: DAPO + PIVOT-v17c on Qwen3-8B-Base.
#
# Pure combination of DAPO's four algorithmic contributions with PIVOT-v17c's
# Langevin rollout. No features from the "Beyond the 80/20 Rule" paper
# (entropy_top_ratio is excluded).
#
# DAPO additions over v17c:
#   - filter_groups (dynamic sampling): drops all-correct / all-wrong batches,
#     guarantees reward variance every step → eliminates dead zone
#   - clip_ratio_high=0.28 (clip-higher): asymmetric PPO clip bounds,
#     larger upper threshold prevents entropy collapse from over-clipping
...
#
# KL: kept at 0.001 (reduced from v17c's 0.005, not zeroed) because
# Langevin perturbations can push the policy away from reference.
#
# PIVOT-v17c Langevin settings unchanged:
#   - trigger-to-trigger G feedback (γ=0.7, α=0.6, no Adam-RMS)
#   - entropy_trigger_only, trig_percentile=65, min_trigger_position=200
set -x
...
CKPT_DIR=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v17c-dapo/qwen3_8b_base

source /storage/workspace/server-1/duy/venv-vault/miniconda3/etc/profile.d/conda.sh
conda activate verl

export VERL_FILE_LOGGER_ROOT=/storage/workspace/server-1/duy/checkpoints/verl/pivot-v17c-dapo
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='verl_pivot_v17c_dapo_qwen3_guru_rl' \
    trainer.experiment_name='qwen3_8b_base_pivot_v17c_dapo' \
