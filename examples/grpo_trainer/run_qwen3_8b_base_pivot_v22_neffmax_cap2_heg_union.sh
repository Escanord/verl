#!/usr/bin/env bash
# DRIFT-v22 neffmax + High-Entropy GRPO (entropy_top_ratio=0.2) on Qwen3-8B-Base, soft-IS ceiling=2.
# UNION variant: gradient acts on (heg top-20% entropy) UNION (Langevin N_eff triggers),
# via +actor.entropy_top_union_trigger=True, so DRIFT soft-IS-corrected trigger tokens are
# not masked out when they fall below heg's entropy cutoff. Fresh run (no resume).
# Scale-up of run_qwen3_4b_base_pivot_v22_neffnoise.sh; matched 1:1 to
# run_qwen3_8b_base_grpo_matched.sh (data / reward / batch 1024 / KL /
# clip 0.2-0.2 / use_kl_in_reward=False / resp_len 16384).
#   - N_eff BAND trigger: fire iff 2 <= 1/Σp² <= 8.
#   - langevin_top_k=128, langevin_exploit_ratio=1.0 (noise OFF, frozen G).
#   - soft-IS: w = min(2, π_old/π_lan_old)  (lan_grpo_softis_wmax=2.0).
# 8B memory (16k): ppo_micro=2, gpu_mem=0.4, max_num_seqs=256, TP=1.
set -x
...
CKPT_DIR=/home/escanord/duy/checkpoints/verl/pivot-v22/qwen3_8b_base_neffmax_cap2_heg_union
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.entropy_top_union_trigger=True \
    ...
    trainer.experiment_name="qwen3_8b_base_pivot_v22_neffmax_cap2_heg_union_${SLURM_JOB_ID:-manual}" \
